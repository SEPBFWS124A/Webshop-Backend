...
public class JobPostingService {
    ...
    public List<JobPostingResponse> getPublicPostings(String employmentType, String city) {
        String type = (employmentType != null && !employmentType.isBlank()) ? employmentType.toUpperCase() : null;
        String loc  = (city != null && !city.isBlank()) ? city : null;
        List<JobPosting> postings;

        if (type != null && loc != null) {
            postings = jobPostingRepository
                    .findByStatusAndEmploymentTypeAndJobLocation_CityOrderByDisplayOrderAsc("ACTIVE", type, loc);
        } else if (type != null) {
            postings = jobPostingRepository
                    .findByStatusAndEmploymentTypeOrderByDisplayOrderAsc("ACTIVE", type);
        } else if (loc != null) {
            postings = jobPostingRepository
                    .findByStatusAndJobLocation_CityOrderByDisplayOrderAsc("ACTIVE", loc);
        } else {
    ...
    @Transactional(readOnly = true)
    public List<JobPostingResponse> getAdminPostings(String status, String employmentType, String city) {
        ...
        if (st != null && type != null && loc != null) {
            postings = jobPostingRepository
                    .findByStatusAndEmploymentTypeAndJobLocation_CityOrderByDisplayOrderAsc(st, type, loc);
            ...
            postings = jobPostingRepository.findByStatusAndJobLocation_CityOrderByDisplayOrderAsc(st, loc);
        } else if (st != null) {
    ...
    private void validateRequest(CreateJobPostingRequest request) {
        ...
        if (request.jobLocationId() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Standort ist ein Pflichtfeld.");
        }
    }
    ...
    private void applyRequest(JobPosting posting, CreateJobPostingRequest request) {
        ...
        JobLocation jobLocation = jobLocationRepository.findById(request.jobLocationId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                        "Standort nicht gefunden: " + request.jobLocationId()));
        posting.setJobLocation(jobLocation);
    }
    ...
    JobPostingResponse toResponse(JobPosting posting) {
        return new JobPostingResponse(
                ...
                posting.getJobLocation() != null ? jobLocationService.toResponse(posting.getJobLocation()) : null);
    }
}
