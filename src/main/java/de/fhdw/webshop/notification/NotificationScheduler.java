 ...
 * US #35 — Notifies sales employees when a product's sales drop by more than
 * 20% week-over-week.
 * US #36 — Notifies sales employees when a purchasable product had zero sales
 * in the past 30 days.
 * #22 — Sends an email alert when the HTTP 5xx error count in 15 minutes
 * exceeds a threshold.
 * #23 — Sends an email alert when JVM heap usage exceeds a configured
    ...
    public void checkHighErrorRate() {
        ...
        if (errorsInInterval >= errorRateThreshold) {
            ...
            try {
                ...
                log.warn("Alert sent: {} HTTP 5xx errors exceeded threshold of {}", (int) errorsInInterval,
                        errorRateThreshold);
    ...
    public void checkJvmHeapUsage() {
        ...
        if (usedPercent >= heapUsageThresholdPercent) {
            ...
            try {
                ...
                log.warn("Alert sent: JVM heap at {}% exceeded threshold of {}%", usedPercent,
